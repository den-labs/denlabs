...
const FACILITATOR_URL =
  process.env.X402_FACILITATOR_URL || "https://facilitator.ultravioletadao.xyz";
...
async function testDevBypass() {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/api/labs/demo-event/retro?format=markdown`,
    );
    ...
    if (response.status === 200) {
      ...
      log(
        `   ${colors.gray}Set X402_DEV_BYPASS=true for development${colors.reset}`,
      );
      return true; // Still valid, just different mode
    } else if (response.status === 404) {
      log(`   ❌ Lab 'demo-event' not found`, "red");
      log(
        `   ${colors.gray}Visit /labs/demo to create the demo lab${colors.reset}`,
      );
      return false;
    } else {
  ...
  try {
    ...
    if (response.status === 402) {
      ...
      try {
        ...
        log(
          `   ${colors.gray}Price: $${payment.price} ${payment.currency}${colors.reset}`,
        );
      ...
      log(
        `   ${colors.gray}To test 402 flow: X402_DEV_BYPASS=false${colors.reset}`,
      );
      return true; // Not a failure, just different mode
    } else if (response.status === 503) {
      log(`   ⚠️  Returned 503 Service Unavailable`, "yellow");
      log(
        `   ${colors.gray}Facilitator is down - this is correct behavior${colors.reset}`,
      );
      return true;
    } else {
      log(`   ❌ Unexpected status: ${response.status}`, "red");
...
async function testConformance() {
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      log(
        `   ${colors.gray}Set X402_DEV_DIAGNOSTICS=true to enable${colors.reset}`,
      );
